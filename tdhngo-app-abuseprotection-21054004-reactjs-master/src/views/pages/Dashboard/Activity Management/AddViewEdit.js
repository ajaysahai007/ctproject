...
export default function CreateNFT(userProfileData) {
  ...
  console.log("_imgDef--->>", _imgDef, "_imageurl--->>", _imageurl);
  const [docType, setDocType] = useState("image");
  // const [_isloading, setLoading] = useState();
  const [_storeId, setStoreId] = useState("");
  const { titleActivity, titleSubActivity } = location.state;
  console.log(
    "titleActivitytitleSubActivity--->>",
    titleActivity,
    titleSubActivity
  );
  ...
  const URLFunction = () => {
    if (functionKey === "Add") {
      return apiConfig.addSleepTrainingData;
    } else {
      return apiConfig.editSleepTrainingData;
    }
  };
  ...
  const AddPeriodSubActivity = async (values) => {
    ...
    formdata.append("videoName", values.title);
  ...
  const ViewPeriodFuntion = async () => {
    try {
      ...
      const res = await axios({
        url: apiConfig.viewSleepTrainingData,
      ...
      if (res) {
        ...
        const formInitialValue = {
          title: res?.data?.result?.videoName,
          content: res?.data?.result?.videoName,
