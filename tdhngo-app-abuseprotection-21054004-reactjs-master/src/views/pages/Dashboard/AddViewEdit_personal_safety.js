...
export default function CreateNFT(userProfileData) {
  ...
  const { Add_send_id, titleActivity, titleSubActivity } = location.state;
  const [_imgDef, setImgDef] = useState("");
  console.log("Add_send_id--->>", titleActivity, titleSubActivity);
  ...
  const URLFunction = () => {
    if (functionKey === "Add") {
      return apiConfig.addPersonalSafetySubData;
    } else {
      return apiConfig.updatePersonalSafetySubData;
    }
  };
  ...
  const AddPeriodSubActivity = async (values) => {
    ...
    // formdata.append("ID", values.title);
  ...
  const ViewPeriodFuntion = async () => {
    try {
      ...
      const res = await axios({
        url: apiConfig.viewPersonalSafetySubData,
      ...
      if (res) {
        ...
        const formInitialValue = {
          ...
          _imgDef: res?.data?.result?.media,
          _imageurl: res?.data?.result?.media,
        };
        setFile(res?.data?.result?.media);
              ...
              <Box mt={5} mb={2} className={classes.main}>
                ...
                >
                  ...
                  {functionKey === "Add" ? (
                    ...
                    <>
                      {/* <Box className={classes.subitBtn}>
                        <Button
                          variant="contained"
                          type="submit"
                          color="primary"
