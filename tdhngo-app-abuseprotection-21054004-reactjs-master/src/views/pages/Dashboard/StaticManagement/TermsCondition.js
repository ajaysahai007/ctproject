...
function Terms() {
  ...
  console.log("location--->>", _type);
  const _dataContent = location?.state?.data;
  const [_isloading, setLoading] = useState(false);
  const [_termAndcondition, setTemAndCondition] = useState();
  // TERM AND CONDITION
  ...
  const TermAndCondition = async () => {
    try {
      ...
      if (res) {
        ...
        setTemAndCondition(res?.data?.result);
      }
    } catch (error) {
      setLoading(false);
              ...
              <Box>
                <Box align="left">
                  <Typography variant="h5" style={{ color: "#000" }}>
                    {convertDateTime(_termAndcondition?.createdAt)}
                  </Typography>
                </Box>
                <Box>
                  <Typography variant="h6" style={{ color: "#000" }}>
                    <div
                      dangerouslySetInnerHTML={{
                        __html: _termAndcondition?.description,
                      }}
                    ></div>
                  </Typography>
                </Box>
