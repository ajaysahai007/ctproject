...
export default function () {
  ...
  const { titleActivity, titleSubActivity, Add_send_id } = location?.state
    ? location?.state
    : "";
  console.log("765726t76234-->>", location?.state);
  //MENTAL HEALTH & WELL BEING
  const ListSleepTraKingData = async () => {
    try {
      setLoading(true);
  ...
  const BlockActivityFunction = async () => {
    ...
    try {
      const res = await axios({
        url: apiConfig.blockSleepTrackingData,
        headers: {
          token: token,
        },
        method: "PUT",
