...
export default function CreateNFT(userProfileData) {
  ...
  console.log("_imgDeffunctionKey--->>", functionKey);
  ...
  const URLFunction = () => {
    if (functionKey === "Add") {
      return apiConfig.addSleepTrackingData;
    } else {
      return apiConfig.editSleepTrackingData;
    }
  };
  //ADD PERIOD_TRACKER MANAGEMENT
  const AddPeriodSubActivity = async (values) => {
    console.log("values---123>>", functionKey);
    console.log("_imgDef--->>", functionKey);
  ...
  const ViewPeriodFuntion = async () => {
    try {
      ...
      const res = await axios({
        url: apiConfig.viewSleepTrackingData,
