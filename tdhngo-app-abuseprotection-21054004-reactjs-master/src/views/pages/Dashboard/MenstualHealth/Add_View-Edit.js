...
export default function CreateNFT(userProfileData) {
  ...
  const { type, Add_send_id, titleSubActivity, titleActivity } = location.state;

  console.log("_imgDeffunctionKey--->>", titleSubActivity, titleActivity);
  ...
  const URLFunction = () => {
    if (functionKey === "Add") {
      return apiConfig.addPeriodTracker;
    } else {
      return apiConfig.editPeriodTracker;
    }
  };
  ...
  const ViewPeriodFuntion = async () => {
    try {
      ...
      const res = await axios({
        url: apiConfig.viewPeriodTracker,
