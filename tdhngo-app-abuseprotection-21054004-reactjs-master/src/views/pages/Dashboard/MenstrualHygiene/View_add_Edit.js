...
export default function CreateNFT(userProfileData) {
  ...
  const [_imageurl_video, setVideoFile] = useState(null);
  ...
  const [_imgDef_video, setImgDef_video] = useState("");
  const { type, Add_send_id, titleActivity, titleSubActivity } = location.state;

  console.log("_imgDeffunctionKey--->>", location.state);
  ...
  const URLFunction = () => {
    if (functionKey === "Add") {
      return apiConfig.addMenstrualHygiene;
    } else {
      return apiConfig.editMenstrualHygiene;
    }
  };
  ...
  const AddPeriodSubActivity = async (values) => {
    ...
    formdata.append("image", _imageurl !== null ? _imageurl : _imgDef);
    formdata.append("objective", values.objective);

    formdata.append(
      "video",
      _imageurl_video !== null ? _imageurl_video : _imgDef_video
    );
    try {
  ...
  const ViewPeriodFuntion = async () => {
    try {
      ...
      const res = await axios({
        url: apiConfig.viewMenstrualHygiene,
      ...
      if (res) {
        ...
        let formInitialValue = {
          ...
          _imageurl_video: res?.data?.result?.video,
          objective: res?.data?.result?.objective,
        };
        setFile(res?.data?.result?.image);
        setVideoFile(res?.data?.result?.video);
                          ...
                          <>
                            ...
                            <figure className={classes.upload}>
                              <ReactPlayer
                                ...
                                url={
                                  _viewperiod?.video ? _viewperiod?.video : ""
                                }
                                width="100%"
                                height="20vh"
                                controls={true}
                          ...
                          <>
                            <figure className={classes.upload}>
                              {/* <img
                                ...
                                controls={true}
                              /> */}
                              <img src={_imgDef === "" ? _imageurl : _imgDef} />
                              <Typography
                                variant="h6"
                                color="primary"
